  ...
  },
  {
    ...
    response: (
      ...
      const isInUser = userModel.some((item) => {
        ...
        if (flag) {
          const { userId: itemUserId, userName, userRole, defaultRoute, authorizedRoutes } = item
          Object.assign(userInfo, { userId: itemUserId, userName, userRole, defaultRoute, authorizedRoutes })
        }
        return flag
      })
